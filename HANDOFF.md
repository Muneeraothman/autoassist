...
- **AWS/infra (built as of Phase 5–7, not "planned" anymore):** S3 (receipt uploads, Phase 5), SES (email verification/password reset, pulled forward from Phase 9 into Phase 4.5), Terraform (`infra/`, Phase 7) provisioning a VPC, RDS, an EC2 instance, an S3-tarball deploy mechanism, and a CloudWatch billing alarm. Three separate IAM identities in use — see the Phase 7 entry for why.
- **Still genuinely not touched:** OpenAI or Bedrock (Phases 10–11 — provider undecided, see §3.6), pgvector, GitHub Actions (Phase 8, up next).
...
- **EC2**: `t3.small`, not the guide's suggested `t3.micro` — deliberate, `docker compose up --build` runs a real Vite/npm build on the instance itself (Phase 8's pre-built-image pipeline doesn't exist yet) and 1GB RAM is genuinely tight for that. App code reaches the instance via an S3 tarball (`deploy/` prefix in the receipts bucket), not a GitHub deploy key — the repo is private, and a tarball avoids provisioning a credential for something Phase 8 will replace shortly anyway. SSH key pair generated by Terraform (app-internal cryptographic material, same treatment as the JWT secret), private key saved locally and git-ignored.
...
2. **Check whether Phase 7's AWS infrastructure is actually running before assuming either way** — this has already bitten the project once (a session said it would `terraform destroy` on wrap-up, didn't, and the infra sat live for an unknown stretch until the next session caught it). Don't trust the Phase 7 entry's "current infra state" note or `terraform state list` alone; confirm directly: `AWS_PROFILE=terraform aws ec2 describe-instances --filters "Name=tag:Name,Values=autoassist-app"` and `aws rds describe-db-instances --db-instance-identifier autoassist-db --region us-east-1`. If anything's running and isn't being actively used, `terraform destroy` it (`cd infra`, `export AWS_PROFILE=terraform`, set `TF_VAR_db_password`/`TF_VAR_jwt_secret_key`, `terraform destroy`).
3. Proceed to **Phase 8 — CI/CD via GitHub Actions**: CI workflow (install deps, run `test_engine.py`/`test_stats.py`, build both Docker images) on every push/PR; a registry to push images to (ECR, adding it to Terraform, or GitHub Container Registry — simpler, no new AWS resource); a CD workflow on merge to `main` that deploys to the EC2 instance (SSH via a GitHub Secret key, or AWS SSM Run Command — no open SSH port needed, more setup effort, offer Muneera the choice per the original guide). GitHub Secrets for all credentials, never in the repo or workflow file. Note this only makes sense while Phase 7's infra is actually up — CD has nothing to deploy *to* if it's been destroyed for the session.
