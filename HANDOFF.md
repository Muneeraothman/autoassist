...
5. Proceed to **Phase 10 — AI assistant**: function-calling over the car's own data (NOT raw SQL generation — the project's explicit security talking point). LLM provider (Bedrock vs. OpenAI) is still genuinely undecided — see §3.6, ask Muneera directly rather than defaulting. Strong candidate for the "Muneera writes the core part herself" treatment (tool-schema design), same as Phase 4's bucketing logic.
