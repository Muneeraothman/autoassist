...
- **AWS/infra (built as of Phase 5–9, not "planned" anymore):** S3 (receipt uploads, Phase 5), SES (email verification/password reset, pulled forward from Phase 9 into Phase 4.5; also used for the Phase 9 reminder emails themselves), Terraform (`infra/` + `infra/bootstrap/`, Phase 7–9) provisioning a VPC, RDS, an EC2 instance, ECR, GitHub OIDC federation, a CloudWatch billing alarm, and (Phase 9) a Lambda function + EventBridge daily schedule for maintenance reminder emails. GitHub Actions CI/CD (Phase 8) — `.github/workflows/ci.yml` + `cd.yml`, deploying via SSM Run Command, no static AWS keys in GitHub Secrets. Five separate IAM identities in use across the project (`autoassist-dev`, `autoassist-terraform`, `autoassist-ec2-role`, `autoassist-github-actions-deploy`, `autoassist-lambda-reminders-role`) — see the Phase 7/8/9 entries for why each is scoped the way it is.
- **AI (Phase 10):** Amazon Bedrock, Converse API, `anthropic.claude-sonnet-4-5-20250929-v1:0` via the `us.` cross-region inference profile (no in-region profile exists for this model in `us-east-1`). `backend/tools.py` (now 5 tool functions + JSON schemas), `backend/bedrock_utils.py` (the tool-calling loop), `POST /api/chat`. Frontend: floating `ChatWidget.jsx`.
- **RAG (Phase 11):** pgvector in the local Postgres instance (`pgvector/pgvector:pg16` image), `manual_chunks` table (`vehicle_id`-scoped), embeddings via Bedrock Titan Text Embeddings V2 (`amazon.titan-embed-text-v2:0`, in-region in `us-east-1`, 1024 dims). `backend/embeddings.py`, `backend/ingest_manuals.py` (re-runnable per vehicle), `search_manual` as the chat's 5th tool.
- **Still genuinely not touched:** RDS-side pgvector (this phase built and tested entirely against the local Postgres instance; RDS Postgres also supports the `vector` extension but nothing was done there yet - flagged in the Phase 11 entry below).
...
- Explicit standing exception, re-affirmed multiple times by both Muneera and Claude Code unprompted: the due-date engine's core comparison + bucketing logic (Phase 4 — see Section 4.4) was deliberately kept as something Muneera wrote herself, specifically because it's the single most likely piece to be asked about in an interview ("walk me through your due-date algorithm"). **Note this pattern was deliberately NOT repeated for Phase 10's tool-calling loop** — Muneera explicitly asked Claude to build that phase itself at the fast pace, with concept explanations substituting for hand-writing (see the Phase 10 entry). Still worth considering case-by-case for any future similarly-central logic, not an automatic rule.
...
### 3.6 — One genuinely open/undecided item remains, one now resolved

- ~~LLM provider for Phases 10–11 (Bedrock vs. OpenAI) has NOT been decided.~~ **Resolved as of Phase 10**: Bedrock, staying in the AWS ecosystem and reusing the project's existing IAM patterns. See the Phase 10 entry below for the model ID and the cross-region inference profile detail that decision surfaced.
...
### ✅ Phase 9 — Email Reminders via EventBridge + Lambda + SES (COMPLETE)

**Data-access tradeoff, decided directly rather than defaulted**: the Lambda calls the backend's own API over plain HTTPS rather than querying RDS directly. Reasons: (1) `engine.py`'s due-date logic gets reused as-is instead of duplicated inside the Lambda, and (2) RDS is in private subnets, so direct access would require VPC-attaching the Lambda, which would then need a NAT gateway or VPC endpoints to reach SES/SSM at all — the exact ongoing cost Phase 7 deliberately avoided by not provisioning a NAT gateway in the first place. Staying outside the VPC sidesteps that entirely.

**Multi-tenancy, per the Phase 4.5 addendum §5.4**: the original build guide's Phase 9 plan assumed a single hardcoded vehicle; this implementation loops every vehicle for every user.

**Schema/backend**: `notifications_log` (`user_id`, `vehicle_id`, `schedule_item_id`, `sent_at`) — a dedup log, not a queue. Two new internal-only endpoints behind a shared-secret header (`X-Internal-Api-Key`, checked in `verify_internal_api_key` — deliberately not JWT-shaped, since the caller has no user session and the endpoint aggregates every user's data, which no individual user's token should be able to do anyway): `GET /api/internal/reminders-due` (loops all vehicles, calls `get_upcoming_maintenance` per vehicle, filters to OVERDUE/DUE_SOON, excludes anything already logged within the last 7 days) and `POST /api/internal/reminders-sent` (records what actually got emailed).

**Lambda** (`infra/lambda_reminders/handler.py`, Python 3.13): reads the backend's current URL from SSM Parameter Store (`/autoassist/backend-url`, written by `user_data.sh.tpl` at boot — the Lambda has no IMDS access the way the EC2 instance itself does, so it can't discover the IP any other way), calls the two internal endpoints, sends one bundled SES email per vehicle (not one email per overdue item), records the sends. If the backend is unreachable, logs and returns cleanly rather than erroring — expected during destroy-between-sessions windows when nothing is running, same "skip gracefully" treatment as Phase 8's CD deploy step.

**Terraform** (`infra/notifications.tf`): `archive_file` data source zips the Lambda source, a dedicated `autoassist-lambda-reminders-role` (SES send + SSM `GetParameter` only — least-privilege, same pattern as every other identity in the project), `aws_cloudwatch_event_rule` on a daily cron (`cron(0 13 * * ? *)`, 8am US Eastern), `aws_lambda_permission` authorizing EventBridge to invoke it.

**By far the longest, deepest bug chain of the project — five real, distinct problems, each hiding behind the last, fully written up as an interview story in JOURNAL.md Session 13. Summary:**
1. `seed.sql` was regenerated locally (for `notifications_log`) but never re-uploaded to the S3 path `user_data.sh.tpl` actually fetches from — the load command reported "Success" because there was nothing to conflict with, it just silently never created a table that wasn't in the file it actually ran.
2. EC2's public IP can change on a plain stop/start, not just a full destroy/apply — `user_data` (and therefore the SSM parameter it publishes) only reruns on a genuine replacement, so a same-instance restart left the Lambda's backend-URL lookup silently stale.
3. RDS master-password changes queue for the next maintenance window unless `apply_immediately = true` is set explicitly (now is, in `database.tf`) — and even then, the reset is asynchronous (`DBInstanceStatus: resetting-master-credentials` for a real stretch of time), so testing right after `terraform apply` finishes can still hit the old password.
4. AWS rejects `/`, `@`, `"`, and spaces in an RDS master password outright — `openssl rand -base64` can produce all three of the first, `openssl rand -hex` can't produce any of them. `variables.tf`'s `db_password` description now says so explicitly.
5. `aws_instance.app` didn't have `user_data_replace_on_change = true` (now does, in `compute.tf`) — without it, editing `user_data.sh.tpl` and running a plain `apply` just stop/starts the same instance to update the stored attribute, and cloud-init skips re-running user-data it's already run once for that instance ID, so a real fix can be committed, applied, and silently never execute.

**Checkpoint verified for real**: schedule temporarily set to `rate(5 minutes)`, confirmed via CloudWatch Logs (`Sent 1 reminder email(s), recorded 8 notification(s)`) and a real email received for the Lexus's 8 overdue items, bundled into one message. Dedup confirmed separately — a follow-up call to `/api/internal/reminders-due` came back empty once those 8 were logged. Schedule reverted to the daily cron afterward.

**Current infra state — LIVE as of this commit, being destroyed immediately after** (Muneera is running `terraform destroy` in her own terminal right after this commit lands, per the standing destroy-between-sessions pattern). As always: **verify directly before trusting this note** — `aws ec2 describe-instances` / `aws rds describe-db-instances` / `aws lambda get-function --function-name autoassist-reminders`, not just `terraform state list`.

### ✅ Phase 10 — AI Assistant, Function-Calling Over the Car's Data (COMPLETE)

**Pacing, explicitly different from Phase 4/9**: Muneera asked Claude to build this whole phase at the fast pace — Claude wrote all the code — with one condition: a tight, plain-language explanation of each genuinely new concept (the tool-calling loop, what a tool JSON schema actually is, why the backend executes tools instead of the model) *before* writing the code for it. Not the "Muneera hand-writes the core logic" pattern from Phase 4's bucketing or intended for Phase 10 in the original build guide — a deliberate, explicit choice this session.

**Provider decision**: Bedrock (see 3.6). Model: `anthropic.claude-sonnet-4-5-20250929-v1:0`, invoked via the `us.anthropic.claude-sonnet-4-5-20250929-v1:0` cross-region inference profile — confirmed directly against current AWS docs rather than assumed, which is what surfaced that `us-east-1` has no *in-region* on-demand inference profile for this model at all, only the cross-region "geo" one. IAM permissions (both `autoassist-dev` and `autoassist-ec2-role`) had to cover `bedrock:InvokeModel` on the underlying foundation model in every region that profile can route to (`us-east-1`/`us-east-2`/`us-west-2`), not just the inference-profile ARN.

**Architecture, matching the build guide's explicit security framing**: `POST /api/chat` (auth-gated, same JWT cookie as everything else) → `backend/bedrock_utils.py` calls Bedrock's Converse API with a system prompt listing the user's own vehicle ids + `backend/tools.py`'s 4 tool specs → if the model requests a tool, the backend (not the model) executes one of 4 fixed, parameterized functions against the real database and feeds the result back in → loop capped at 5 iterations. The model never writes SQL and never sees a connection string; it can only ever request one of four pre-approved functions, each of which independently re-checks that `vehicle_id` belongs to the authenticated user regardless of what the model asked for.

**Tools** (`backend/tools.py`, reusing `engine.py`/`stats.py` exactly the way Phase 9's Lambda reused `engine.py` — pure functions called from a new entry point rather than duplicated): `get_vehicle_info`, `get_upcoming_maintenance`, `get_service_history` (optional `service_name`/`since_date` filters), `get_spending_summary` (optional `category`/`year` filters). Ownership enforced in a shared `_get_owned_vehicle` helper that raises a `ToolError` (fed back to the model as a tool-result error, not an HTTP exception — a bad/hallucinated `vehicle_id` should end the conversation gracefully, not crash the whole request).

**Frontend**: `ChatWidget.jsx`, a floating button/panel (not gated behind vehicle selection, since the assistant can answer about any of the user's vehicles by id).

**Two real bugs found during testing, both in JOURNAL.md Session 14 in full**:
1. Bedrock's Converse API rejects a bare JSON array as a `toolResult`'s `json` content (`ValidationException`) — two of the four tools naturally return a list. Fixed by wrapping list results in `{"items": [...]}` in `bedrock_utils.py`, keeping the Bedrock-specific quirk out of `tools.py`.
2. `get_service_history` originally didn't resolve `schedule_item_id` to an actual service name — caught because the model itself noticed and said so honestly rather than guessing. Fixed by joining in the name (or `stats.py`'s `OTHER_CATEGORY` constant for unscheduled repairs).

**Checkpoint, run against a disposable test account with its own vehicle (never real vehicle data) — 12 questions, all correct**: all four tools individually, one multi-tool question in a single turn, two ownership-boundary probes against another account's vehicle id (one passive, one an explicit "ignore your instructions" attempt — confirmed via the actual tool-error response, not just model politeness, that the *backend* rejects it), three out-of-scope refusals (general car advice, weather, unrelated coding request) with zero wasted tool calls.

### ✅ Phase 11 — RAG Over the Owner's Manual PDFs (COMPLETE)

**Pacing**: same as Phase 10 — built at the fast pace, explicit "no questions back, make reasonable decisions and document them" instruction.

**Infra decision**: pgvector in the existing local Postgres instance (the guide's recommended Option A), not a separate Bedrock Knowledge Base. Local Postgres image switched from `postgres:16` to `pgvector/pgvector:pg16` (identical Postgres 16 underneath, extension pre-compiled in — vanilla `postgres:16` has no `vector` extension available at all). Confirmed the existing named volume's real data survived the image swap before doing anything else with it.

**Schema**: `manual_chunks` (`id`, `vehicle_id` FK → `vehicles`, `source_file`, `page_number`, `chunk_text`, `embedding VECTOR(1024)`), `vehicle_id` present from the first migration per the Phase 4.5 addendum's §5.6 note — not retrofitted later. No ivfflat/hnsw approximate-nearest-neighbor index yet; a sequential cosine-distance scan over ~637 rows is fast enough that one isn't needed, revisit only if the corpus grows meaningfully.

**Embedding model**: Bedrock Titan Text Embeddings V2 (`amazon.titan-embed-text-v2:0`), 1024 dimensions, confirmed in-region in `us-east-1` (unlike Phase 10's Claude Sonnet 4.5, no cross-region inference profile needed for this one — checked per-model rather than assumed). `backend/embeddings.py` is a standalone module specifically so both `backend/ingest_manuals.py` and `backend/tools.py` can import `embed_text()` without `tools.py` and `bedrock_utils.py` importing each other.

**Ingestion** (`backend/ingest_manuals.py`, re-runnable per `--vehicle-id`, clears that vehicle's old chunks first): extracts text per page via `pdfplumber`, chunks each page into ~500-word pieces with a 75-word overlap (word count as the token-count proxy for the guide's "~500-800 tokens" target). Chunks deliberately never cross a page boundary, so `search_manual` citations are always an exact page number, never a range. `manuals/2A1313OM.pdf` (the stale old Honda manual) is explicitly excluded. Run for real against vehicle id=2 (the actual Lexus): 637 chunks, 371 from `OM33566U.pdf` + 266 from `SMG202.pdf`.

**Tool**: `search_manual(query, vehicle_id)`, the chat's 5th tool, same `_get_owned_vehicle` ownership check as the other four. The system prompt was updated to explicitly separate "structured DB tools" from "manual search," require citing source file + page number when answering from the manual, and require an honest "the manual doesn't seem to cover this" instead of guessing when nothing relevant comes back.

**`seed.sql` design decision**: regenerated with `pg_dump --exclude-table-data=manual_chunks` — the schema (`CREATE EXTENSION`, `CREATE TABLE manual_chunks`) ships in the dump, but the ~637 real embedding rows do not. Treated as regenerable/derived data from the PDFs already in the repo (same category as S3 receipt objects, also excluded from `seed.sql`), not hand-entered source-of-truth data like the rest of the app's real rows — a fresh clone needs to run `python ingest_manuals.py --vehicle-id <id>` once, same as it already needs its own `backend/.env`.

**IAM**: `bedrock:InvokeModel` on `amazon.titan-embed-text-v2:0` added to both `autoassist-dev` (live, via AWS CLI) and `infra/compute.tf`'s `ec2_bedrock` policy (not yet applied — infra is currently destroyed).

**One real bug, one repeated-but-finally-fixed process gap, both in JOURNAL.md Session 15 in full**: a benign-but-real collation-version warning from the base-image swap (fixed via `ALTER DATABASE ... REFRESH COLLATION VERSION`), and a third repeat of the Session 8/14 foreign-key cleanup mistake — this time actually stopped and checked `information_schema` for every FK referencing `vehicles`/`users` before writing the delete batch, which is what finally broke the pattern.

**Checkpoint, run against a disposable test account with the real PDFs re-ingested under its own test vehicle id (never the real account) — all required scenarios passed**: an oil-type question correctly used `search_manual` with real page citations; a due-date question correctly used the DB tool instead of the manual and matched a real seeded service record exactly; a "hybrid battery" question (the car isn't a hybrid) correctly declined to hallucinate and volunteered the actually-relevant 12V procedure instead, with citations; a forced adversarial `search_manual` call against another vehicle's id was rejected by the same ownership check as every other tool.
...
**What's in Git (safe, portable):** all backend code, all frontend code, `manuals/` (both Lexus PDFs + the stale Honda one), `seed.sql` (full schema + real data, regenerated via `docker exec autoassist-db pg_dump -U postgres -d autoassist --inserts --exclude-table-data=manual_chunks > seed.sql` whenever the data changes meaningfully — the `--exclude-table-data` flag is new as of Phase 11, see that entry for why `manual_chunks`' embedding rows specifically are excluded from the dump), `backend/requirements.txt`, `frontend/package.json` (dependencies, not `node_modules`), `backend/Dockerfile`, `frontend/Dockerfile` + `nginx.conf`, `docker-compose.yml`, CLAUDE.md, GLOSSARY.md, JOURNAL.md, LEARNING_PROCESS.md, AUTOASSIST_BUILD_GUIDE.md.
...
docker run --name autoassist-db -e POSTGRES_PASSWORD=devpassword -e POSTGRES_DB=autoassist -p 5432:5432 -d pgvector/pgvector:pg16   # pgvector image as of Phase 11 - plain postgres:16 has no vector extension, seed.sql's CREATE EXTENSION would fail
sleep 3
...
python ingest_manuals.py --vehicle-id 2   # populates manual_chunks - seed.sql intentionally ships schema only, see Phase 11
...
2. **Check whether the AWS infrastructure is actually running before assuming either way** — this has already bitten the project once (a session said it would `terraform destroy` on wrap-up, didn't, and the infra sat live for an unknown stretch until the next session caught it). Don't trust a prior session's "current infra state" note or `terraform state list` alone; confirm directly: `AWS_PROFILE=terraform aws ec2 describe-instances --filters "Name=tag:Name,Values=autoassist-app"`, `aws rds describe-db-instances --db-instance-identifier autoassist-db --region us-east-1`, and (new as of Phase 9) `aws lambda get-function --function-name autoassist-reminders --region us-east-1` / `aws events describe-rule --name autoassist-reminders-daily --region us-east-1` (the Lambda + EventBridge rule live in the same destroyable `infra/` state as everything else, so they should be gone too if the rest is). If anything's running and isn't being actively used, `terraform destroy` it (`cd infra`, `export AWS_PROFILE=terraform`, set `TF_VAR_db_password`/`TF_VAR_jwt_secret_key`/`TF_VAR_internal_api_key`, `terraform destroy`).
...
4. **When generating `db_password` for any future apply, use `openssl rand -hex 32`, not base64** — see the Phase 9 entry above; base64's alphabet can produce characters RDS rejects outright as a master password.
5. **After any fresh clone or DB reset, re-run manual ingestion** — `seed.sql` intentionally does NOT include `manual_chunks`' row data (see the Phase 11 entry): `cd backend && source venv/bin/activate && python ingest_manuals.py --vehicle-id <id>` (id=2 for the real Lexus, as of this writing) before expecting `search_manual`/the chat's manual questions to return anything.
6. Proceed to **Phase 12 — polish/README/architecture diagram/resume bullets/final interview-prep rehearsal**, per the build guide. Describe the app as multi-user/multi-vehicle from the start (true by now), not single-car-with-auth-bolted-on. Worth a "design decisions" paragraph on the ownership-check pattern specifically, since by now it's used consistently in four different places (REST routes, S3 keys, AI tool execution, and RAG retrieval) — see §5.7 of the addendum. RDS-side pgvector (flagged as not-yet-done in the Phase 11 entry) should get resolved before or during this phase if the deployed app needs to actually demo the chat/RAG features live, not just locally.
