...
- **Phase 11:** RAG over the actual Lexus owner's manual PDFs (already in the repo, `manuals/OM33566U.pdf` and `manuals/SMG202.pdf`) — pgvector in Postgres is the guide's recommended approach, embeddings via Bedrock Titan Embeddings (now that Bedrock is the settled provider from Phase 10).
...
5. Proceed to **Phase 11 — RAG over the Lexus owner's manual PDFs**: pgvector in the existing RDS Postgres instance (guide's recommended Option A), embeddings via Bedrock Titan Embeddings (Bedrock is now the settled provider — see Phase 10). Check whether Titan Embeddings needs its own explicit Bedrock model-access step beyond what Phase 10 already enabled for Claude Sonnet 4.5 — don't assume it's automatically covered.
